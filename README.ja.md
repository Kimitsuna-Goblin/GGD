...
なお、混合比の和を常に $1$ にするための正規化項 $1 - \Phi_1(x) + \Phi_2(x)$ は
使用しないことにします。その目的は定義式を単純化し、積分を容易にするためです。
混合ガウス分布とは異なり、グラデーションガウス分布では、混合率の正規化は必須ではありません。
...
のように漸増、漸減すると言えます。
なお、混合比の和を常に $1$ にするための正規化項 $1 - f_1(x)/f_1(\mu_1) + f_2(x)/f_2(\mu_2)$ は
水平 (横方向) と同様の理由で、使用しません。
