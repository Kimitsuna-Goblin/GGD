...
いわゆる **混合ガウス分布 (Gaussian Mixture Model; GMM)** 、すなわち、正規分布の一次結合で表されるモデルとは異なり、
...
$\mu_1$ と $\mu_2$ の値が異なる場合、
$\mathcal{N}_1$ と $\mathcal{N}_2$ の取り方によっては、
$\mathcal{G}[\mathcal{N}_1 \uparrow \mathcal{N}_2]$ の確率密度関数の頂点は
$f_2$ の頂点よりも $f_1$ の頂点に近い位置に来ることがあり得ます。
上のほうの文章で、  $\mathcal{G}[\mathcal{N}_1 \uparrow \mathcal{N}_2]$ の
確率密度関数の変化のしかたについて、「少なくともイメージとして」と書いたのはこれが理由です。
