...
X座標が $-\infty \to \infty$ と漸増するにつれて、確率密度関数の形状が
正規分布 $\mathcal{N}_1$ のものから正規分布 $\mathcal{N}_2$ のものに徐々に変化するような
...
ここで、 $\mu_1$ と $\mu_2$ は必ずしも等しい値である必要はないことに注意してください。
通常、 $\mathcal{G}[\mathcal{N}_1 \uparrow \mathcal{N}_2]$ の確率密度関数の形状は
確率密度関数 $f_1$ の山の裾の形から、徐々に $f_2$ の山の形に近づいていって、
頂上は $f_2$ の頂上と同じか、またはそこに近い位置に来るイメージです。
ところが、 $\mu_1$ と $\mu_2$ の値が異なる場合、
$\mathcal{N}_1$ と $\mathcal{N}_2$ の取り方によっては、
...
これらのケースは「縦方向グラデーション」の直感的なイメージには反するかも知れませんが、許容されます。
...
のように定義できます。
ただし、 $h_1(x)$ と $h_2(x)$ は横方向グラデーション分布の混合比を表す関数です。
...
ただし、 $N_{i,j}$ は正規分布です。
$v_{i,1}(x)$ 、 $v_{i,2}(x)$ は縦方向グラデーション分布の混合比を表す関数です。
