...
| ジェネレータ  | nls.freq.all      | サポートしている全種類の分布モデルで度数分布の近似を試みます。    |
| ジェネレータ  | trace.q           | クォンタイルをトレースするGGD クラスオブジェクトを生成します。    |
| メソッド      | GGD$d             | 確率密度関数の値を返します。                                      |
| メソッド      | GGD$p             | X座標 (あるいはクォンタイル) に対する確率を返します。             |
| メソッド      | GGD$q             | 確率に対するX座標 (あるいはクォンタイル) を返します。             |
| メソッド      | GGD$r             | 分布モデルに従うランダムサンプルを返します。                      |
| メソッド      | GGD$sd, usd, lsd  | 標準偏差、上側標準偏差、下側標準偏差を返します。                  |
| メソッド      | GGD$tex           | 確率密度関数と累積分布関数を TeX 形式で表示します。               |
