...
累積密度関数でクォンタイルをトレースさせる関数 GGD$set.waypoints() では、
最大8点のクォンタイルを通過させることができます。
このことは、例えば、確率 $\lbrace 0.1, 0.25, 0.4, 0.5, 0.6, 0.75, 0.9 \rbrace$ のクォンタイルを
 (エラーが生じなければ) ほとんど誤差なくトレース可能であることを意味します。
