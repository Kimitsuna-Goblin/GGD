...
以下の式において、 $\Phi^\ast_i(x)$ は平均値が $\mu_i$ 、標準偏差が $\sigma_i / \sqrt{2}$ の正規分布の累積分布関数を表します。
...
$f_2(x)$ の負担率は $f_2(x)$ の値が増加するにつれて $0$ から徐々に増加します。

ただし、それぞれの正規分布の平均値 $\mu_1, \mu_2, \mu_3$ が異なる場合は、
$f_2(x)$ の負担率の最大値は $1$ にはなりませんし、
$f_1(x)$ や $f_3(x)$ の負担率が、ある点から一時的に増加に転じ、再び減少することがあります。
