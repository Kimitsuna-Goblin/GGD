...
確率密度関数 $f(x)$ 、累積分布関数 $\Phi(x)$ は以下の式で表されます。
...
確率密度関数 $g(x)$ 、累積分布関数 $\Psi(x)$ は
2つ、あるいは3つの正規分布の確率密度関数 $f_1(x), f_2(x), f_3(x)$
と累積分布関数 $\Phi_1(x), \Phi_2(x), \Phi_3(x)$ を使って、以下の式で表されます。
...
このクラスでは、
確率密度関数 $g(x)$ は2つ、または3つの正規分布の確率密度関数 $f_1(x), f_2(x), f_3(x)$
によって負担されます。
...
確率密度関数 $g(x)$ 、累積分布関数 $\Psi(x)$ は
4つの正規分布の確率密度関数 $f_{1,1}(x), f_{1,2}(x), f_{2,1}(x), f_{2,2}(x)$
と累積分布関数 $\Phi_{1,1}(x), \Phi_{1,2}(x), \Phi_{2,1}(x), \Phi_{2,2}(x)$ を使って、以下の式で表されます。
