...
累積密度関数でクォンタイルをトレースする関数では、3点または4点のクォンタイルをトレースできます。
例えば、確率 $\lbrace 0.25, 0.5, 0.75 \rbrace$ のクォンタイルを
 (正規分布に従わない形、すなわち、左右対称でない形で、かつ、過度にいびつな形でなければ)
ほとんど誤差なくトレースできます。
...
累積密度関数でクォンタイルをトレースする関数では、最大6点のクォンタイルをトレースできます。
ただし、平均値のクォンタイルは四分位等ではなく、山部と裾部の代表となる点を選ぶべきです。
例えば、確率 $\lbrace 0.1, 0.4, 0.5, 0.6, 0.9 \rbrace$ のクォンタイルを
...
累積密度関数でクォンタイルをトレースする関数では、最大8点のクォンタイルをトレースできます。
